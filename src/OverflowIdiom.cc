...
Value *OverflowIdiom::matchCmp(CmpInst::Predicate Pred, Value *L, Value *R)
{
	...
	if (Pred == CmpInst::ICMP_NE && match(L, m_Value(X)) && match(R, m_UDiv(m_Mul(m_Value(A), m_Value(B)), m_Value(Y))) && ((A == X && B == Y) || (A == Y && B == X)))
	...
	if (Pred == CmpInst::ICMP_UGT && match(L, m_Value(X)) && match(R, m_UDiv(m_ConstantInt(C), m_Value(Y))))
	{
