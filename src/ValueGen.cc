	...
	struct ValueVisitor : InstVisitor<ValueVisitor, SMTExpr>
	{
		...
		SMTExpr visitBinaryOperator(BinaryOperator &I)
		{
			...
			case Instruction::UDiv:
				return SMT.bvudiv(L, R);
			case Instruction::SDiv:
				return SMT.bvsdiv(L, R);
			case Instruction::URem:
				return SMT.bvurem(L, R);
			case Instruction::SRem:
				return SMT.bvsrem(L, R);
			case Instruction::Shl:
				return SMT.bvshl(L, R);
			case Instruction::LShr:
				return SMT.bvlshr(L, R);
			case Instruction::AShr:
				return SMT.bvashr(L, R);
			case Instruction::And:
				return SMT.bvand(L, R);
			case Instruction::Or:
				return SMT.bvor(L, R);
			case Instruction::Xor:
				return SMT.bvxor(L, R);
			}
		}
		...
		SMTExpr visitICmpInst(ICmpInst &I)
		{
			...
			case CmpInst::ICMP_SGE:
				return SMT.bvsge(L, R);
				break;
			case CmpInst::ICMP_SGT:
				return SMT.bvsgt(L, R);
				break;
			case CmpInst::ICMP_SLE:
				return SMT.bvsle(L, R);
				break;
			case CmpInst::ICMP_SLT:
				return SMT.bvslt(L, R);
				break;
			case CmpInst::ICMP_UGE:
				return SMT.bvuge(L, R);
				break;
			case CmpInst::ICMP_UGT:
				return SMT.bvugt(L, R);
				break;
			case CmpInst::ICMP_ULE:
				return SMT.bvule(L, R);
				break;
			case CmpInst::ICMP_ULT:
				return SMT.bvult(L, R);
				break;
			}
		}
...
void addRangeConstraints(SMTSolver &SMT, SMTExpr E, MDNode *MD)
{
	...
	{
		const APInt &Lo = mdconst::dyn_extract<ConstantInt>(MD->getOperand(i))->getValue();
		const APInt &Hi = mdconst::dyn_extract<ConstantInt>(MD->getOperand(i + 1))->getValue();
		/* 		const APInt &Lo = mdconst::dyn_extract<ConstantInt>(MD->getOperand(i))->getValue();
		const APInt &Hi = mdconst::dyn_extract<ConstantInt>(MD->getOperand(i + 1))->getValue(); */
