...
void RangePass::collectInitializers(GlobalVariable *GV, Constant *I)
{
	...
	if (ConstantStruct *CS = dyn_cast<ConstantStruct>(I))
	{
		...
		if (!ST->hasName() || ST->getName() == "struct.anon" || ST->getName().startswith("struct.anon."))
			return;
...
CRange RangePass::visitCastInst(CastInst *CI)
{
	...
	{
	case CastInst::Trunc:
		return getRange(BB, V).zextOrTrunc(bits);
	case CastInst::ZExt:
		return getRange(BB, V).zextOrTrunc(bits);
	case CastInst::SExt:
		return getRange(BB, V).signExtend(bits);
	case CastInst::BitCast:
		return getRange(BB, V);
	default:
...
bool RangePass::visitCallInst(CallInst *CI)
{
	...
	for (FuncSet::iterator i = CEEs.begin(), e = CEEs.end(); i != e; ++i)
	{
		...
		if ((*i)->isVarArg() || (*i)->getName().find('.') != StringRef::npos)
			continue;
